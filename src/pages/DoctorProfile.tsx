...
export default function DoctorProfile() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* HEADER */}
      <div className="bg-[color:var(--brand)]" style={{ ["--brand" as any]: brand }}>
        <div className="mx-auto w-full max-w-lg px-4 pb-6 pt-5">
          <div className="relative overflow-hidden rounded-2xl">
            <img src={doctor.avatar} alt="" className="h-56 w-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
            <div className="absolute bottom-4 left-4 right-4 text-white">
              <div className="text-2xl font-extrabold leading-tight">
                Dr. {doctor.firstName} {doctor.lastName}
              </div>
              <div className="mt-1 text-white/90">{doctor.clinic || "Orzu Medical"}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* STICKY CTA */}
      <div className="sticky bottom-0 mt-6 border-t border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex w-full max-w-lg items-center gap-3 px-4 py-3">
          <button
            onClick={() => setOpen(true)}
            className="grid h-12 flex-1 place-items-center rounded-xl text-white font-semibold"
